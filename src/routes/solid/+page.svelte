<script lang="ts">
  ...
  const filteredIcons = filterIconArrayByKeyword(data.props.iconNames, 'Solid');
  let searchTerm = '';
  ...
  <Tabs style="pill" {contentClass} class="p-4">
    <TabItem open>
      ...
      <div class="grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8 px-4 dark:text-white">
        ...
        <div class="flex gap-4 items-center text-lg">
          <IconSolid name={iconName} bind:width={size} bind:height={size} class="shrink-0"/>
          {iconName}
        </div>
    ...
    <TabItem>
      ...
      <div class="grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8 px-4 dark:text-white">
        ...
        <div class="flex gap-4 items-center text-lg">
          <IconSolid name={iconName} bind:width={size} bind:height={size} color={random_hex_color_code()} class="shrink-0"/>
          {iconName}
        </div>
    ...
    <TabItem>
      ...
      <div class="grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8 px-4 dark:text-white">
        ...
        <div class="flex gap-4 items-center text-lg">
          <IconSolid name={iconName} bind:width={size} bind:height={size} class={random_tailwind_color()} />
          {iconName}
        </div>
