...
const title = 'Mini Icons - Svelte Heros v2';
...
test('Mini Icons page has expected h1, meta title', async ({ page }) => {
  await expect(page.getByRole('heading', { name: title })).toBeVisible();
});

test('Mini Icons page has expected meta title', async ({ page }) => {
  await expect(page).toHaveTitle(title);
});

test('Mini Icons page has expected meta description', async ({ page }) => {
  const metaDescription = page.locator('meta[name="description"]');
  await expect(metaDescription).toHaveAttribute('content', description);
});

test('Mini Icons page has expected meta og', async ({ page }) => {
...
test('Mini Icons page has expected meta twitter', async ({ page }) => {
